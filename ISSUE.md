...
**Also broaden the message.** It attributes the 409 to a `stray 'bun server.ts' process or a second session` — both *local*, and both things you would go looking for with `pgrep`. That framing sent me hunting locally, and it is exactly the wrong instinct when the local sweep comes back empty: the token may be held from somewhere you cannot reach — another host, a container, a machine you forgot you deployed it on — and no `pgrep`, no orphan sweep, and no `bot.pid` check can see such a holder. Crucially, **a clean local sweep does not rule one out either** (see "Why a 409 is hard to attribute" above), so an operator who trusts the current wording is left concluding that nothing is holding the token while something demonstrably is. Naming the unreachable case, and pointing at `/revoke` as the only way to break it, is the difference between a dead end and a fix. Suggest: `…(a stray poller, a second session, or the same token running on another host)`.
